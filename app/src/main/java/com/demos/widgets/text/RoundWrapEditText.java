 ...
 * description :
 */
public class RoundWrapEditText extends AppCompatEditText {
    ...
    public RoundWrapEditText(@NonNull Context context) {
        super(context);
        init();
    }

    public RoundWrapEditText(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public RoundWrapEditText(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void drawOtherRight(Canvas canvas, int index, RectF currentRect, RectF beforeRect) {
        ...
        path.lineTo(currentRect.right, currentRect.bottom - bottomOffset);

        int gravity = getGravity();
        if ((gravity & Gravity.LEFT) == Gravity.LEFT || (gravity & Gravity.CENTER) == Gravity.CENTER) {
            if (currentRect.right >= beforeRect.right) {
                path.lineTo(beforeRect.right, currentRect.bottom - bottomOffset);
                path.lineTo(beforeRect.right, beforeRect.bottom - bottomOffset);
            } else {
                path.lineTo(currentRect.centerX() - 2f, currentRect.bottom - bottomOffset);
                path.lineTo(currentRect.centerX() - 2f, beforeRect.bottom - bottomOffset);
            }
        } else {
            path.lineTo(currentRect.right - radius - 2f, currentRect.bottom - bottomOffset);
    ...
    private void drawOtherLeft(Canvas canvas, int index, RectF currentRect, RectF beforeRect) {
        ...
        path.lineTo(currentRect.left, currentRect.bottom - bottomOffset);
        ...
        if ((gravity & Gravity.LEFT) == Gravity.LEFT || (gravity & Gravity.CENTER) == Gravity.CENTER) {
            ...
            path.lineTo(currentRect.right - radius, currentRect.bottom - bottomOffset);
